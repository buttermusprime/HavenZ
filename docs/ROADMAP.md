...
Sizing a display box to "match" another element without deriving it from native resolution × a shared scale factor produces mismatched sprites ("mixels"). HavenZ mixes 16px characters with much larger vehicle/building tiles — this is a real risk, not a hypothetical one. Addressed directly in **Phase 2.1** (locking the constant in HavenZ's PixelPipe config) and structurally in PixelPipe's own Build Guideline 01.
