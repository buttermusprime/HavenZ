...
| 5.1 | Movement cards — Stealth & Loud | Done | 2026-08-29 | S4.4 left movement targeting with no mechanism; asked the user rather than guessing, who chose reintroducing click-to-target over baking direction into cards. noise_cost now carries a dual meaning (flat vs. per-tile rate) per CardResource's own pre-existing S0.2 doc comment. Found a real forgotten-CSV-reimport bug (same class as S4.3's) via screenshot. |
...
**19 of 61 pre-ship sessions done, 1 deferred, 41 not started** (plus POST.1). Real elapsed time: 5 active calendar days (2026-08-25 through 2026-08-29), 31 commits, 24 real working sittings against those 19 sessions.
